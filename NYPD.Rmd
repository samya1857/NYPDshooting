...
nypd_r %>% ggplot(aes(x = PERP_RACE, y = count))+geom_line()+
...
x_grid <- seq(25000, 35000)
new_df <- tibble(sum_employees = x_grid)
df_joined_w <- df_joined %>% mutate(pred = predict(mod))

df_joined_w %>% ggplot() + geom_point(aes(x = sum_employees, y= count), color="blue") + 
  geom_point(aes(x = sum_employees, y=pred), color = "red")
```
